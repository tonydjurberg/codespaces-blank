...
def parse_card(card):
    ...
    for line in lines[1:]:
        if re.search(r"\\b\\d{3}\\s?\\d{2}\\b",line):
            ...
            if m:
                postcode=clean(m.group(1)); city=clean(m.group(2))
            break
        if re.search(r"\\bStockholm\\b",line,re.I) and not address:
            address=line
    parts=name.split()
    return {
    ...
    with sync_playwright() as p:
        ...
        for n in range(1,MAX_PAGES+1):
            ...
            try:
                ...
                print("Navigation error:",e); continue
            ...
            before=len(rows)
            # Candidate links are deliberately restricted to public Eniro person pages.
            links=page.locator("a").all()
            for link in links:
                try:
                    href=link.get_attribute("href") or ""
                    if not re.search(r"(person|privatperson)",href,re.I): continue
